...
import { render, View, Grid, BlockStack, useCartLines, Heading, Image, useExtensionApi, Spinner } from "@shopify/checkout-ui-extensions-react";
...
function App() {
    ...
    useEffect(async () => {
        await fetch(`/api/v1/merchant/config/${shopId}`)
            .then((resp) => resp.json())
    ...
    async function getCalculations() {
        ...
        if (calcEndpoint.length > 1) {
            ...
            await fetch(`myServerurl/api/v1/carbon-accounting/calculate/${calcEndpoint}`, {
                method: "POST",
                headers: {
            ...
            <View
                ...
                <Grid
                    ...
                    <View>
                        <BlockStack>
                            {recommerce
                                ? `With this purchase you give pre-owned items a second life. You help avoiding up to ${Math.abs(
                                      co2EmissionsTotal.toFixed(2)
                                  )} kg CO2 emissions`
                                : `This order causes up to ${co2EmissionsTotal.toFixed(
                                      2
                                  )} kg CO2eq (product + transport) Together with For the Earth we estimate emissions of every transaction`}
                            <Grid columns={["fill", "auto"]}>
                                <Heading level={3}>{recommerce ? `CO2 emissions avoided` : `CO2 emissions`}</Heading>
                                <Heading
