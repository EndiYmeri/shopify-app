...
import { extend, render, useExtensionInput, BlockStack, View, Button, Heading, Image, Separator, InlineStack, Text, Layout, TextBlock, Tiles, TextContainer, Button } from '@shopify/post-purchase-ui-extensions-react';
...
async function getRenderData(shopId) {
  const response = await fetch(`https://fortheearth-api.cartediem.org/api/v1/merchant/config/${shopId}`)
    .then(resp => resp.json())
    .then(merchant => {
      return merchant
    })
...
async function sendCheckoutData(data, merchantConfigInfo) {
  ...
  await fetch('https://fortheearth-api.cartediem.org/api/v1/conscious-order', {
    "method": "POST",
    "headers": {
...
export function App() {
  ...
  return (
    <BlockStack spacing="loose" alignment="center">
      ...
      <View>
        <TextBlock size="large" emphasized>
          {
          ...
          {storage?.initialData?.response?.plan.planId === "DONATION-1" ? 'Congrats! You made your order carbon-neutral by supporting certified offsetting projects.' : ""}
        </TextBlock>
      </View>
      <Layout maxInlineSize={500}>
        {
          storage?.initialData?.response?.plan?.planId === "CARBON-OFFSETTING-1"
            ? <Image fit="cover" source="https://cdn.shopify.com/s/files/1/0677/4567/7593/files/43A1692.jpg?v=1669996252" /> : null
        }
        {
          storage?.initialData?.response?.plan?.planId === "REFORESTATION-1"
            ? <Image fit="cover" source="https://cdn.shopify.com/s/files/1/0677/4567/7593/files/GC24609wm.jpg?v=1669995859" /> : null
        }
        {
      ...
      <View>
        {
          lineItems.map((item) => {
            ...
            return (
              <Layout maxInlineSize={400} media={["small"]} key={item.product.id} sizes={[250, 100]} spacing="extraLoose">
      ...
      <Layout maxInlineSize={400} sizes={[250, 100]} spacing="xloose">
        <TextContainer alignment="leading">
      ...
      <Layout maxInlineSize={400} sizes={[250, 100]} spacing="xloose">
        ...
        <TextContainer alignment="trailing" >
          <TextBlock emphasized>
            {
              inputData.initialPurchase.totalPriceSet.shopMoney.amount
              + ' ' +
              inputData.initialPurchase.totalPriceSet.shopMoney.currencyCode
            }
          </TextBlock>
